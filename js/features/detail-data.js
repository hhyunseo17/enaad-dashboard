...
// ============================================================
    ...
    function renderDetailDataPivot() {
      ...
      if (multiValue) {
        const valueRowCells = [];
        visibleColumns.forEach(() => { valueDefs.forEach(v => valueRowCells.push(`<th style="text-align:center; font-size:12px; font-weight:700;">${getDetailDataValueLabel(v)}</th>`)); });
        rows.push(valueRowCells);
        const lastIdx = rows.length - 1;
        valueDefs.forEach(v => rows[lastIdx].push(`<th style="text-align:center; font-size:12px; font-weight:500; background:#1E40AF !important; color:#FFFFFF !important;">${getDetailDataValueLabel(v)}</th>`));
      }
