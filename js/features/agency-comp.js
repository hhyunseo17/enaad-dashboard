    ...
    function renderAgencyCompChart() {
      ...
      chartInstances.agencyComp = new Chart(ctx, {
        ...
        data: {
          labels: topGroups,
          datasets: [
            { label: `전년동월(${py}.${cm})`, data: topGroups.map(g => getVal(prevYearMap, g)), backgroundColor: CH('#3A4258'), borderRadius: 4,
            ...
            { label: `전월(${pmY}.${pmM})`, data: topGroups.map(g => getVal(prevMonthMap, g)), backgroundColor: CH('#6B7280'), borderRadius: 4,
              datalabels: { display: 'auto', anchor: 'end', align: 'top', color: dataLabelTextColor(), font: { family: 'Pretendard', size: 10, weight: '700' }, formatter: (v) => v > 0 ? v.toFixed(1) : '' }
            },
            { label: `당월(${cy}.${cm})`, data: topGroups.map(g => getVal(currMap, g)), backgroundColor: CH('#60A5FA'), borderRadius: 4,
              datalabels: { display: 'auto', anchor: 'end', align: 'top', color: dataLabelTextColor(), font: { family: 'Pretendard', size: 10, weight: '700' }, formatter: (v) => v > 0 ? v.toFixed(1) : '' }
            }
          ]
        },
