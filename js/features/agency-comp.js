...
// ============================================================
    ...
    function renderAgencyCompChart() {
      ...
      chartInstances.agencyComp = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            { label: `전년동월(${py}.${cm})`, data: topGroups.map(g => getVal(prevYearMap, g)), backgroundColor: ddBarFill(RC('ref')), borderRadius: 5, ...ddGroupSeparator(), barPercentage: 1, categoryPercentage: 0.8,
              datalabels: { display: 'auto', anchor: 'end', align: 'top', color: dataLabelTextColor(), font: { size: 11, weight: FW() }, formatter: (v) => v > 0 ? v.toFixed(1) : '' }
            },
        ...
        options: {
          ...
          scales: { y: ddValueAxis({ ticks: { color: CH('#8B95A1'), maxTicksLimit: 5, padding: 6, callback: v => v + (agencyCompMetricMode === 'revenue' ? '억' : '') } }), x: { ticks: { color: CH('#F2F4F6'), font: { size: 12, weight: FW() } }, grid: { display: false } } }
        }
      });
    }
    ...
    const AC_STYLES = [
      ...
      { rowClass: '', label: 'background:#0D1117; color:#64748B;',
        num: 'text-align: right;', cur: 'text-align: right;', w: '' },
      { rowClass: '', label: 'background:#090C10; color:#475569; font-size:13px;',
        num: 'text-align: right;', cur: 'text-align: right;', w: '' },
    ];
