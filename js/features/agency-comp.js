    ...
    function renderAgencyCompChart() {
      ...
      chartInstances.agencyComp = new Chart(ctx, {
        ...
        options: {
          ...
          scales: { y: ddValueAxis({ ticks: { color: CH('#8B95A1'), maxTicksLimit: 5, padding: 6, callback: v => v + (agencyCompMetricMode === 'revenue' ? '억' : '') } }), x: { ticks: { color: CH('#F2F4F6'), font: { family: 'Pretendard', size: 11, weight: '400' } }, grid: { display: false } } }
        }
      });
    }
