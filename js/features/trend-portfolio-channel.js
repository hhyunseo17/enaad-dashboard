...
// ============================================================
    function renderTrendChart() {
      ...
      const datasets = categories.map(cat => {
        ...
        return { label: cat, data: data, backgroundColor: ddBarFill(catColor(cat) || chartColors.blue), borderRadius: 0,
          datalabels: {
    ...
    function renderPortfolioChart() {
      ...
      chartInstances.portfolio = new Chart(ctx, { type: 'doughnut', data: { labels: labels, datasets: [{ // 세그먼트 분리는 라운드/spacing이 아니라 '카드 배경색 테두리'로 낸다.
        ...
        data: dataVals, backgroundColor: bgColors, borderWidth: 1.5, borderColor: ddSurfaceColor(), borderAlign: 'inner', hoverOffset: 6,
        datalabels: {
