...
// ============================================================
      ...
      const datasets = categories.map(cat => {
        ...
        return { label: cat, data: data, backgroundColor: ddBarFill(catColor(cat) || chartColors.blue), borderRadius: 0, ...ddStackSeparator(),
          datalabels: {
            ...
            anchor: 'end', align: 'top', offset: 4, color: dataLabelTextColor(), font: { family: 'Pretendard', size: 12, weight: '500' },
    ...
    function renderPortfolioChart() {
      ...
      chartInstances.portfolio = new Chart(ctx, { type: 'doughnut', data: { labels: labels, datasets: [{ // 세그먼트 분리는 라운드/spacing이 아니라 '카드 배경색 테두리'로 낸다.
        ...
        datalabels: {
          display: 'auto', color: '#FFFFFF', font: { family: 'Pretendard', size: 12, weight: '500' }, textStrokeColor: 'rgba(0,0,0,0.22)', textStrokeWidth: 1.5,
          formatter: (value) => `${((value / totalSum) * 100).toFixed(1)}%`
        }
      }] }, options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 24 } }, cutout: '68%', plugins: { legend: { position: 'right', labels: { color: CH('#B0B8C1'), font: { size: 11, family: 'Pretendard', weight: '500' } } }, tooltip: { callbacks: { label: (ctx) => `${ctx.label}: ${ctx.raw.toFixed(2)} 억원 (${((ctx.raw / (dataVals.reduce((a,b)=>a+b,0)||1))*100).toFixed(1)}%)` } } } } });
    }
      ...
      const datasets = categoryOrderList.map(cat => {
        ...
        return { label: cat, data: data, backgroundColor: ddBarFill(catColor(cat) || chartColors.blue), borderRadius: 0, ...ddStackSeparator(),
          datalabels: {
            ...
            anchor: 'end', align: 'top', offset: 4, color: dataLabelTextColor(), font: { family: 'Pretendard', size: 11, weight: '500' },
            formatter: (value, ctx) => { let total = 0; ctx.chart.data.datasets.forEach(ds => { total += ds.data[ctx.dataIndex] || 0; }); return total > 0 ? total.toFixed(1) + '억' : ''; }
          }
        };
      });
    ...
    function renderAdvBucketChart() {
      ...
      chartInstances.advBucket = new Chart(ctx, {
        ...
        data: { labels: labels, datasets: [ { type: 'bar', label: '광고주 수', data: countVals, backgroundColor: ddDuoFill(catColor('일반광고'), catColor('IMC')), yAxisID: 'y', borderRadius: 6, order: 2,
          datalabels: { display: 'auto', anchor: 'end', align: 'top', color: dataLabelTextColor(), font: { family: 'Pretendard', size: 11, weight: '500' }, formatter: (v) => v > 0 ? v + '개' : '' }
          ...
          datalabels: { display: 'auto', anchor: 'end', align: 'top', offset: 8, color: '#FFB547', font: { family: 'Pretendard', size: 11, weight: '500' }, formatter: (v) => v > 0 ? v.toFixed(1) + '억' : '' }
        } ] },
