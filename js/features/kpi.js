    ...
    function renderGoalTrendChart() {
      ...
      chartInstances.goalTrend = new Chart(ctx, {
        ...
        data: { labels: labels, datasets: [
          ...
          { label: '목표', data: targetVals, backgroundColor: ddBarFill(CH('#3A4258')), borderRadius: 5, ...ddGroupSeparator(), barPercentage: 1, categoryPercentage: 0.8,
            datalabels: { display: false }
          },
          { label: '실적', data: actualVals, backgroundColor: ddDuoFill('#4795FF', '#7B6BF0'), borderRadius: 5, ...ddGroupSeparator(), barPercentage: 1, categoryPercentage: 0.8,
        ...
        options: {
          ...
          scales: { x: { ticks: { color: CH('#F2F4F6'), font: { family: 'Pretendard', size: 12, weight: '600' } }, grid: { display: false } }, y: ddValueAxis({ ticks: { color: CH('#8B95A1'), maxTicksLimit: 5, padding: 6, callback: v => v + '억' } }) }
        }
      });
    }
