...
// ============================================================
    ...
    //
    // 예전엔 Tailwind의 서로 다른 계열에서 하나씩 집어온 값이라 세트로 조율된 적이 없었다.
    // 채도와 명도가 제각각이면 개별 색은 선명해도 모아놨을 때 '기본 팔레트' 느낌이 난다.
    // 지금은 색상(hue)만 유지하고 채도·명도를 서로 맞춰 한 가족으로 보이게 조정했다.
    //
    //   일반광고  파랑   브랜드 블루(--primary-blue와 동일). 가장 큰 비중이라 기준점 역할.
    //   IMC       인디고 파랑과 이웃하되 확실히 갈리는 보라. 파랑 다음 비중.
    //   인포머셜  에메랄드 예전 #10B981은 어두운 배경에서 형광끼가 돌아 한 단계 눌렀다.
    //   큐톤광고  골드   예전 #F59E0B는 노랑 쪽으로 너무 떠서 주황 쪽으로 당기고 눌렀다.
    //   기타광고  중립   어드레서블·콘텐츠편성 등을 흡수한 '잔여' 버킷이라 무채색이 의미상 맞다.
    //                    (예전 #22C55E는 인포머셜과 사실상 같은 초록이라 구분되지 않았다.)
