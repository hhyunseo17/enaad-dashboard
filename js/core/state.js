...
// ============================================================
    ...
    //
    // **IMC와 인포머셜은 사용자 요청으로 색을 맞바꿨다(빨강 ↔ 제이드).** 아래 색별 조율 근거는
    // 전부 유효하다 — 값은 그대로고 어느 분류가 갖느냐만 바뀌었다. 바뀐 것은 스택에서 어느
    // 색끼리 맞닿느냐다. categoryOrderList 순서로 쌓이므로 맞닿는 쌍은 일반–IMC / IMC–인포 /
    // 인포–큐톤 셋이고, 실측(OKLCH, 색상차° / 명도차L)은 다음과 같다.
    //
    //              교환 전                        교환 후
    //   일반–IMC   133° / L3.9  (라 136 / L2.3)   85° / L14.6  (라  81 / L10.9)
    //   IMC–인포   143° / L10.7 (라 143 / L13.2)  143° / L10.7 (라 143 / L13.2)  ← 불변
    //   인포–큐톤  104° / L1.2  (라 100 / L1.8)   39° / L11.9  (라  43 / L15.0)
    //
    // **교환은 색상차를 명도차로 바꿔치기한다.** 맞닿는 쌍의 최소 색상차는 100~104° → 39~43°로
    // 크게 좁아지지만, 최소 명도차가 1.2~1.8 → 10.1~10.9로 올라간다. 교환 전 팔레트는
    // "색상은 멀지만 명도는 사실상 같은" 구성이라 색상 하나에 의존했다(그래서 예전 주석이
    // 색상차를 그렇게 따졌다). 교환 후에는 맞닿는 모든 쌍이 명도로도 10 이상 벌어져,
    // 구분 근거가 두 축으로 나뉜다. 어느 쪽이 낫다고 단정할 문제가 아니다.
    //
    // 다만 빨강(인포머셜)과 주황(큐톤광고)이 39~43°로 팔레트에서 가장 가까운 쌍이 되었고
    // 하필 스택에서 맞닿는다는 점은 남는다. 지금은 명도차 12~15가 받쳐 주어 화면에서 구분되지만
    // **이 쌍의 명도를 좁히는 방향으로는 손대지 말 것** — 색상차가 이미 얇아 명도까지 붙이면
    // 한 덩어리로 읽힌다. 더 벌려야 한다면 명도가 아니라 인포머셜 빨강을 장미 쪽(350° 부근)으로
    // 옮기는 것이 맞다.
    ...
    // 아래 라이트 값은 대부분 그 계산이 만들어내던 값 그대로다. 빨강(현 인포머셜)만 다르다 —
    // 계산 결과는 #FF7F77(L73)이었는데, 빨강은 그 명도에서 분홍·살몬으로 읽힌다. 명도를 고를 수
    // 있게 된 김에 L62로 내려 빨강이 빨강으로 보이게 했다. 나머지 계열(L63~64)과 같은 대역이고,
    // 흰 카드 대비도 2.50에서 3.29로 올라간다.
    //
    // **제이드(현 IMC)는 135°가 아니라 165°다.** System Green(135°)은 노랑 쪽으로 기운
    ...
    // **여기서 더 올리지 말 것.** 세 단계(0.140)면 주황(0.162)에 근접해 제이드 계열이 비중에
