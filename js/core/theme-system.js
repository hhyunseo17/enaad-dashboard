...
// ============================================================
    ...
    const SERIES_ROLES = {
      ref:   { light: '#B0B8C1', dark: '#4B5A6C' },  // 목표 · 전년동월 · MoM 전월
      prev:  { light: '#778494', dark: '#94A3B8' },  // 대행사 전월
      curr:  { light: '#0064FF', dark: '#60A5FA' },  // 대행사 당월
      // 구간별 분포 차트의 합산 매출액 선. 라이트를 #CF7B00(대비 3.24)까지 내려 봤는데,
      // 주황은 명도를 내리면 갈색이 된다 — 선도 그 아래 영역 채움도 탁한 베이지로 읽혔다.
      // 채도가 가장 높은 지점(L50)으로 되돌린다. 대비는 2.20으로 하한 아래지만 선명하다.
      // 다크는 카드가 어두워 밝은 끝을 그대로 쓸 수 있으므로 값이 다르다.
      line:  { light: '#FF9500', dark: '#FFB547' },
      // 전월대비 발산형 램프(신규 → 증액 → 유지 → 감액 → 중지).
      // 중간 두 단계는 흰 배경 대비 2.2로 하한 아래지만 그대로 둔다. 채도를 유지한 채
      // 명도만 내리면 초록은 이끼색, 주황은 갈색이 되어 램프 전체가 탁해진다 —
      // 실제로 한 번 내려 봤고(#2EB04F / #E08300) 갈색으로 읽혔다. 선명함을 택한다.
      momNew:  { light: '#009A4E', dark: '#4ADE80' },
      momUp:   { light: '#34C759', dark: '#2FA97A' },
      momFlat: { light: '#778494', dark: '#94A3B8' },
      momDown: { light: '#FF9500', dark: '#E08A5F' },
      momStop: { light: '#FF3B30', dark: '#F87171' }
    };
    function RC(role) {
