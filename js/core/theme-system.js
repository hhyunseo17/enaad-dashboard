...
// ============================================================
    ...
    const CHART_COLOR_MAP = {
      ...
      '#F87171': '#FF3B30', '#FBBF24': '#FF9500',
      // MoM 발산형 램프(신규→증액→유지→감액→중지)용.
      // 예전에는 라이트 배경에서 채도를 깎고 명도를 떨어뜨렸는데(증액 S79 L27, 감액 S66 L46),
      // 그러면 초록이 이끼색, 주황이 갈색으로 읽혀 탁해진다. 이 파일 아래 ddLift와 같은 원칙 —
      // 채도는 두고 명도만 조절 — 으로 맞춘다. 기준은 대행사 비교 차트의 당월 파랑(S100 L50)이다.
      // 양 끝(신규·중지)이 진하고 중간(증액·감액)이 옅은 발산형이 되도록 명도를 배치한다.
      '#4ADE80': '#009A4E', '#2FA97A': '#34C759', '#E08A5F': '#FF9500'
    };
    ...
    // [시범] 5대분류를 파랑 단색 계조로만 표현하는 대안 팔레트. 현재 월별 매출 추이 차트에서만 쓴다.
    //
    // categoryOrderList가 이미 매출 규모 순(일반광고 → … → 기타광고)이라, 단색 계조로 두면
    // 색이 '어느 분류인가'와 '얼마나 큰가'를 동시에 말한다. 기준은 대행사 비교 차트의 당월 파랑이다.
    //
    // 기타광고만 램프에서 빼고 중립 회색으로 둔다. 어드레서블·콘텐츠편성 등을 흡수한 잔여 버킷이라
    // 파랑 계조의 한 칸을 차지하는 게 의미상 맞지 않고, 램프 끝을 흰색까지 늘리면 흰 카드에서
    // 사라진다(큐톤광고 1.42:1이 이미 하한이다).
    //
    // 계조는 ddSoften을 태우지 않는다 — 명도 차이가 곧 분류 구분인데 전부 같이 밝히면 그 차이가 준다.
    //
    // 단계 간격은 앵커(일반광고)를 내려서 번다. 위로는 못 늘린다 — 큐톤광고가 흰 카드 대비
    // 1.48:1이라 더 밝히면 카드에 묻힌다. 그래서 아래로 벌어야 5단이 다 들어간다.
    // 라이트 앵커는 L26까지 내려 실제 네이비가 됐다(색상 216°·채도 100%는 기준 파랑과 동일).
    // 인접 대비 최소 1.86:1 — 1.35(붙어 보임) → 1.63 → 1.86으로 두 번 벌린 결과다.
    const CAT_MONO_LIGHT = { '일반광고': '#003585', 'IMC': '#0664E0', '인포머셜': '#5F9FE7', '큐톤광고': '#C5D6E7', '기타광고': '#898E94' };
    // 다크는 반대로 위쪽으로 번다. 짙은 카드에서는 앵커를 더 내릴 수 없어서(배경에 묻힌다)
    // 밝은 끝을 L91까지 올렸다. 인접 최소 1.50:1로 라이트보다 낮은데, 어두운 배경에서는
    // 같은 명도 차이가 더 크게 읽혀 실사용에서는 비슷하게 갈린다.
    const CAT_MONO_DARK  = { '일반광고': '#0062F5', 'IMC': '#4592F7', '인포머셜': '#9BC2ED', '큐톤광고': '#DFE8F1', '기타광고': '#85898E' };
    function catColorMono(name) {
      return (currentTheme === 'light' ? CAT_MONO_LIGHT : CAT_MONO_DARK)[name];
    }
    ...
    // strength: 그라데이션 세기 배수(기본 1). 단색 계조 팔레트처럼 **세그먼트 사이의 명도 차이 자체가
    // 정보인** 경우에는 이 값을 낮춘다. 기본 세기는 세그먼트 안에서 명도가 18포인트 움직이는데,
    // 계조 단계 간격이 11~14포인트라 그대로 두면 안쪽 그라데이션이 단계 차이를 덮어
    // 스택 전체가 하나의 연속된 띠로 뭉개진다.
    function ddBarFill(hex, horizontal, strength) {
      ...
      return (ctx) => {
        ...
        const base = currentTheme === 'light' ? ddLift(hex, -0.10 * k) : ddLift(hex, 0.12 * k);
        const tip  = currentTheme === 'light' ? ddLift(hex,  0.08 * k) : ddLift(hex, -0.04 * k);
