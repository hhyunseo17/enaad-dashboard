...
// ============================================================
    ...
    const CHART_COLOR_MAP = {
      ...
      // 참조 막대(목표·전년동월). 예전 짝은 #3A4258 → #D1D6DB였는데 흰 카드 대비 1.46,
      // 다크 카드 대비 1.71로 두 테마 모두에서 사실상 빈 공간이었다. 물러나 있는 역할은
      // 유지하되 보이기는 해야 하므로 2.3~2.4까지만 올린다. 채도는 건드리지 않았다.
      '#4D5875': '#A2ACB6', '#6B7280': '#8B95A1', '#60A5FA': '#0064FF', '#94A3B8': '#8B95A1',
      '#F87171': '#FF3B30', '#FBBF24': '#FF9500',
      // 채널 차트의 합산 매출액 선. 표에 아예 없어서 라이트에서 다크 값(#FFB547, 1.76)이
      // 그대로 그려지고 있었다. 선은 면보다 얇아 대비가 더 필요하므로 3.2까지 올린다.
      '#FFB547': '#CF7B00',
      ...
      // 중간 두 단계(증액·감액)가 2.22 / 2.20으로 하한 아래였다. 양 끝보다 옅다는 발산형의
      // 의도는 유지하면서 2.8까지만 올린다.
      '#4ADE80': '#009A4E', '#2FA97A': '#2EB04F', '#E08A5F': '#E08300'
    };
