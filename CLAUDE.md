...
| 지표 대시보드 (경쟁채널 벤치마크 — CPRP/시청률/GRP/M-S) | `js/core/metrics-data-loader.js`(연결·파싱·ENA 치환) + `js/features/metrics-dashboard.js`(매출/M-S) + `js/features/metrics-ratings.js`(CPRP/시청률/GRP+상세표) | `docs/features/metrics-dashboard.md` |
| 상세 피벗 (항목/부서/담당자/채널/광고주/대행사별) | **`js/features/pivot-builder.js`의 `PIVOT_PRESETS`** — 여섯 개 전부 이관됨. `detail-pivots.js`의 원본 렌더러는 `USE_PIVOT_ENGINE=false` 되돌리기용으로만 남아 있다 | `docs/features/detail-pivots.md` |
| 피벗 엔진 (행 N단계 × 열 N단계 × 값 복수, 프리셋) | `js/features/pivot-builder.js` | `docs/features/detail-pivots.md` |
...
9. **차트 색은 테마별로 값을 따로 적는다.** 한쪽에서 계산해 다른 쪽을 만들지 않는다 —
   `js/core/theme-system.js`의 `SERIES_ROLES`·`CHART_COLOR_MAP`·`PIVOT_COLOR_MAP`,
   `js/core/state.js`의 `categoryColorsLight/Dark`가 모두 `{ light, dark }` 형태다.
   (과거에 `ddSoften`이 라이트 = 정의값+0.14로 만들어냈고, 룩업 표는 다크 값을 키로 썼다.
   그 구조에서는 한 테마만 손보는 것이 불가능했다.)
10. **라이트는 탁하면 안 되고, 다크는 라이트의 반전이 아니다.** 두 테마 색은 비슷하되 각자 값이다.
    - 라이트: 유채색 채도 60% 이상. 대비를 벌리려고 명도를 내리면 초록은 이끼색, 주황은
      갈색이 된다. **대비보다 선명함이 우선**이라 3.0 미만인 채움이 여럿 있고, 의도한 것이다.
    - 다크: '물러남'을 어둡게 만드는 것으로 표현하지 않는다. 카드에 가까워져 구멍처럼 보인다.
      옆 색보다 덜 밝은 정도면 충분하다.
    - 두 테마 모두 **자기 5대분류 명도대 안**에 둔다(라이트 L62~73, 다크 L50~67).
      대역을 벗어난 색은 같은 화면의 나머지보다 무겁거나 떠 보인다.
    - 색상(hue)은 화면에 이미 있는 것만 쓴다: 파랑 211 · 빨강 3~5 · 초록 165(서수 팔레트는 155) · 주황 35 · 무채.
      초록은 135(System Green)에서 옮겼다 — 노랑 쪽으로 기운 초록이라 나머지 넷 사이에서
      혼자 연두로 떠 보였다. 밝기 문제가 아니므로 명도로 고치려 하지 말 것(내리면 이끼색이 된다).
      (서수 팔레트만 예외 — 부서가 8개 이상이라 10색이 필요하다.)
