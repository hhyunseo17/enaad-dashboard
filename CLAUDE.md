...
| 매출분류·필터 규칙 (본부매출/5대분류/취급고·회계 등) | `js/core/filters.js` (**commonMatch 단일 지점**) | `docs/data-rules.md` |
| 데이터 적재·파싱·Supabase·ETL | `js/core/data-loader.js` | `docs/data-rules.md` |
...
5. **필터링·집계는 `commonMatch`(js/core/filters.js) 경유.** 새 compute 함수를 만들 때도 이걸 재사용하고, 본부매출+실적 필터를 우회하지 말 것.
...
## 장기 로드맵 (Supabase 전환)
- `js/core/data-loader.js`가 **프론트/백 경계**. Supabase 전환 시 이 파일만 "엑셀 파싱 → supabase 쿼리"로 교체하고, `rawData`/`filteredData`의 형태(shape)는 유지한다. 나머지 `features/*`는 건드리지 않는다.
- `commonMatch` → SQL VIEW `v_bonbu_sales`(본부매출+실적) + `v_reclassified`(5대분류)로 승격.
- K2병합·업프론트 파싱 → 적재 시점 ETL로 이동(런타임 계산에서 제거).
- Zero Trust로 접근통제 중이므로 프론트가 Supabase client로 직접 쿼리(1안) 권장. Worker는 정적 서빙 유지.
