...
| `metricsEnsureDefaultSelections()` | metrics-dashboard.js | 최초 렌더 시 연도/①사업자 기본값 채움 — ①사업자는 "범위" 토글이 가리키는 전체(`metricsAllOperatorGroups()`)로 채운다(2026-09-16, top4 랭킹 로직 폐지 — 사용자가 고른 뒤로는 건드리지 않음) |
