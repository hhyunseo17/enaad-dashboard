...
- Supabase 접근은 반드시 `/api/*` 프록시(Pages 배포 시 `functions/api/*.js`) 경유 — 브라우저가 데이터를 Supabase(`*.supabase.co`)에서 직접 쿼리하지 않는다. 개인별 인증은 Supabase Auth(`js/core/auth.js`)가 맡고, 로그인 세션의 JWT를 `/api/*` 요청마다 `Authorization: Bearer`로 실어 보내면 프록시가 검증한다(아래 "인증 게이트" 참고). anon(publishable) key는 이 로그인 SDK 전용이며 데이터 쿼리 권한이 없다.
